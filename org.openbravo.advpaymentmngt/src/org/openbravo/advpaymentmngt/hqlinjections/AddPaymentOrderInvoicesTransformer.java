...
public class AddPaymentOrderInvoicesTransformer extends HqlQueryTransformer {
  ...
  protected StringBuffer getSelectClause(String transactionType, boolean hasSelectedIds) {
    ...
    /* RARC BEGIN */
    selectClause
        .append(" COALESCE(ips.ssdnidCollectionExpenses, 0) AS EM_Ssdnid_CollectionExpenses, ");
    selectClause
        .append(" COALESCE(ips.ssdnidInterestLatePayment, 0) AS EM_Ssdnid_InterestLatePayment, ");
    selectClause.append(
        " COALESCE(ips.sSDNIDTCollectionInterestfForCollection, 0) AS EM_Ssdnid_TCollectionExpenses, ");
    selectClause.append(
        " COALESCE(ips.sSDNIDTInterestCollectionForLatePayment, 0) AS EM_Ssdnid_TInterestLatePayment, ");
    selectClause.append(" ips.sSDNIDGeneratePayment AS EM_Ssdnid_GeneratePayment, ");
    selectClause.append(" COALESCE(ips.ssdnidPayment,'') AS EM_Ssdnid_Payment, ");
    /* RARC END */
    selectClause.append(" bp.id as businessPartner, ");
    selectClause.append(" bp.name as businessPartnerName, ");
    selectClause.append(
        " SUM(psd.amount + psd.writeoffAmount - (case when fp.status = 'RPAE' then COALESCE(fp.amount,0) else 0 end)) as outstandingAmount, ");
  ...
  protected StringBuffer getGroupByClause(String transactionType) {
    ...
    /* RARC BEGIN */
    groupByClause.append(" ips.ssdnidCollectionExpenses, ");
    groupByClause.append(" ips.ssdnidInterestLatePayment, ");
    groupByClause.append(" ips.sSDNIDTCollectionInterestfForCollection, ");
    groupByClause.append(" ips.sSDNIDTInterestCollectionForLatePayment, ");
    groupByClause.append(" ips.sSDNIDGeneratePayment, ");
    groupByClause.append(" ips.ssdnidPayment.id, ");
    /* RARC END */
